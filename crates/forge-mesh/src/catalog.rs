...
pub struct ModelInfo {
    ...
    pub frontier: bool,
    /// Genuinely free (priced at $0 and not a subscription bridge): local/ollama, gateway `:free`.
    pub free: bool,
    /// A $0-marginal subscription CLI bridge (claude-cli/codex-cli).
    pub subscription: bool,
    /// Estimated USD for a nominal turn (0 = free/subscription/unpriced).
    pub cost: f64,
}
...
impl ModelInfo {
    fn classify(id: &str, pricing: &Pricing) -> Self {
        ...
        Self {
            ...
            free: !subscription && cost <= f64::EPSILON,
            subscription,
            cost,
        }
    }
...
impl ProviderGroup {
    ...
    pub fn paid(&self) -> usize {
        self.models.iter().filter(|m| m.cost > f64::EPSILON).count()
    }
}
...
impl ModelCatalog {
    ...
    pub fn stats(&self, pricing: &Pricing) -> CatalogStats {
        ...
        CatalogStats {
            ...
            paid: infos.iter().filter(|m| m.cost > f64::EPSILON).count(),
        }
    }
...
mod tests {
    ...
    fn overview_catalog() -> ModelCatalog {
        ModelCatalog::new(vec![
            ...
            "openai::gpt-4o-mini".into(),           // mid, paid
            "groq::llama-3.1-8b-instant".into(),    // small, free (unpriced)
    ...
    fn stats_count_each_category() {
        ...
        assert_eq!(s.total, 6);
        assert_eq!(s.providers, 5); // anthropic, openai, groq, ollama, claude-cli
        assert_eq!(s.frontier, 2); // opus + groq-70b
        assert_eq!(s.subscription, 1); // claude-cli
        assert_eq!(s.free, 3); // groq-8b, groq-70b, ollama
        assert_eq!(s.paid, 2); // opus, gpt-4o-mini
    }
