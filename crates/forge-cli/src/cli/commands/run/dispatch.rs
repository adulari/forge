        ...
        CommandAction::Statusline(action) => {
            ...
            let widget_name = |w: &forge_config::StatuslineWidget| -> String {
                match w {
                    ...
                    forge_config::StatuslineWidget::QuotaOpencodeGo => "quota_opencode_go".into(),
            ...
            match action {
                ...
                StatuslineAction::Toggle(widget_name_str) => {
                    let widget = match widget_name_str.as_str() {
                        ...
                        "quota_opencode_go" | "opencode_go" | "go" => {
                            forge_config::StatuslineWidget::QuotaOpencodeGo
                        }
                        ...
                        other => {
                            app.note(&format!(
                                "⚠ unknown widget '{other}' — use: model, tier, session_cost, \
                                 ...
                                 quota_codex, quota_opencode_go, quota_pace, mcp_status \
                                 (custom/shell widgets are \
...
`spawn_agents` instead — that's what it's for. Parse agent output defensively: agents return \
prose, so ask discovery agents for one item per line with no commentary and no code fences, \
split on newlines (never JSON.parse free-form prose), filter out empty or 'null' items before \
fanning out, and never interpolate a possibly-null value into an agent prompt. Always end the \
script with `return <final result>` so the run yields a relayable answer.\n\nGoal: {goal}"
                        ),
                        guidance: Vec::new(),
                        tier: Some(forge_types::TaskTier::Complex),
                    });
