...
pub use dispatch_store::{
    dispatch_item_status, dispatch_status, DispatchItemRow, DispatchRow, NewDispatchItem,
};
...
mod tests {
    ...
    fn schedule_roundtrips_list_last_run_and_remove() {
        ...
        assert!(store.set_schedule_enabled(&id, false).unwrap());
        assert!(!store.list_schedules().unwrap()[0].enabled);
        assert!(store.set_schedule_enabled(&id, true).unwrap());
        assert!(store.list_schedules().unwrap()[0].enabled);
    ...
    #[test]
    fn a_late_queue_finisher_cannot_overwrite_a_terminal_outcome() {
        let store = Store::open_in_memory().unwrap();
        store
            .add_queue_task("q1", "task", "/repo", None, None, None)
            .unwrap();
        assert!(store.claim_queue_task("q1", 10).unwrap());
        store
            .finish_queue_task("q1", "done", 20, None, None, Some("first"), None, None)
            .unwrap();
        store
            .finish_queue_task("q1", "failed", 30, None, None, Some("late"), None, None)
            .unwrap();

        let row = store.list_queue_tasks(None).unwrap().remove(0);
