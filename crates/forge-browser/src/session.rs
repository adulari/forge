//! The live browser session: one Chrome, one page, its capture, interception, and replay.
...
use crate::network::{self, Exchange, Filter, NetworkLog};
use crate::{cdp, har, truncate, LOAD_TIMEOUT, MAX_BODY_CHARS};

/// A live browser: one Chrome, one page, and everything that page fetched.
pub struct BrowserSession {
    process: BrowserProcess,
    client: Arc<cdp::CdpClient>,
...
impl BrowserSession {
    ...
    /// log in by hand once and have every later turn drive the session they left open. A re-attach
    ...
    async fn attach_to(process: BrowserProcess, fingerprint: &launch::Fingerprint) -> Result<Self> {
        ...
        let rules = Arc::new(Mutex::new(intercept::InterceptionRules::default()));
            ...
            while let Some(event) = events_rx.recv().await {
                if event.method.starts_with("Network.") {
                    ...
                    *pump_loaded
                    ...
                    // Every paused request MUST be resolved or the page hangs, so this is spawned
                    // rather than awaited inline: a slow resolve for one request cannot stall the
                    // reader for the next event.
                    resolve_paused(
                        Arc::clone(&pump_client),
                        Arc::clone(&pump_rules),
                        event.params,
                    );
                }
            }
        ...
        for (method, params) in launch::fingerprint_commands(fingerprint) {
            session
                .client
                .call(method, params)
                .await
                .with_context(|| format!("apply fingerprint override {method}"))?;
        }
        Ok(session)
    }
    ...
    /// The current capture as a HAR 1.2 document.
    ...
    /// Install an interception ruleset. Enables `Fetch` if the rules are non-empty and disables it
    /// when they are empty, so a page is only paused-per-request while interception is actually
    /// wanted.
    pub async fn set_interception(&self, rules: InterceptionRules) -> Result<()> {
        ...
        *self
        ...
        match (was_empty, empty) {
            // Turning interception on: pause all requests so the pump can apply the rules.
            (true, false) => {
                self.client
                    .call("Fetch.enable", json!({"patterns": [{"urlPattern": "*"}]}))
                    .await?;
            }
            // Turning it off: stop pausing, so the page runs at full speed again.
            (false, true) => {
                self.client.call("Fetch.disable", json!({})).await?;
            }
            // Rules changed but interception stays on/off: nothing to toggle.
            _ => {}
        }
        Ok(())
    }

    /// Clear all interception rules and stop pausing requests.
    pub async fn clear_interception(&self) -> Result<()> {
        self.set_interception(InterceptionRules::default()).await
    }

    /// Re-issue a request from inside the page, reusing its cookies and origin, and return the
    /// response. This is the core reverse-engineering loop: capture what the page sent, change one
    /// thing, and see what the server does.
    ///
    /// It runs as an in-page `fetch` with `credentials: 'include'`, so it carries the session the
    /// user is logged into — which is the point — but it is also therefore subject to the page's
    /// CORS rules. Same-origin (the API you are on) always works; a cross-origin target the server
    /// does not allow will fail as it would for any script on that page, and that failure is
    /// reported rather than hidden.
    pub async fn replay(&self, request: &ReplayRequest) -> Result<Value> {
        ...
        // The whole fetch is built in JS from JSON-encoded arguments, so a URL or header with a
        // quote cannot break out of the expression.
        let script = format!(
            "(async () => {{ \
               const r = await fetch({url}, {init}); \
               const headers = {{}}; r.headers.forEach((v, k) => {{ headers[k] = v; }}); \
               let body = ''; try {{ body = await r.text(); }} catch (e) {{ body = '<unreadable: ' + e + '>'; }} \
               return {{ status: r.status, statusText: r.statusText, headers, \
                        body: body.slice(0, {cap}), type: r.type, url: r.url }}; \
             }})()",
        ...
        self.eval(&script)
            .await
            .with_context(|| format!("replay {} {}", request.method, request.url))
    }
}

/// A request to re-issue from inside the page.
...
/// Resolve one paused request per the active rules. Spawned, never awaited by the reader, because
/// a paused request that is never answered hangs the page and one slow resolve must not delay the
/// next event.
fn resolve_paused(
    client: Arc<cdp::CdpClient>,
    rules: Arc<Mutex<InterceptionRules>>,
    params: Value,
) {
    tokio::spawn(async move {
        let Some(fetch_id) = params.get("requestId").and_then(Value::as_str) else {
            return;
        };
        let url = params
            .get("request")
            ...
            .unwrap_or_default();
        let request_headers =
            network::header_pairs(params.get("request").and_then(|r| r.get("headers")));
        let decision = rules
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .decide(url, &request_headers);

        let _ = match decision {
            intercept::Decision::Block => {
                client
                    .call(
                        "Fetch.failRequest",
            ...
            intercept::Decision::Continue => {
                client
                    .call("Fetch.continueRequest", json!({"requestId": fetch_id}))
                    .await
            }
            intercept::Decision::ContinueWithHeaders(headers) => {
                let cdp_headers: Vec<Value> = headers
                    .into_iter()
                ...
                client
                    .call(
                        ...
                        json!({"requestId": fetch_id, "headers": cdp_headers}),
                    )
                    .await
            }
        };
