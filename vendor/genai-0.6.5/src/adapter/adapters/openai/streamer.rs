...
impl OpenAIStreamer {
	...
	fn capture_tool_call(&mut self, index: usize, call_id: String, fn_name: String, arguments: String) -> ToolCall {
		...
		if let Some(existing_call) = calls.get_mut(index) {
			...
			// New tool call at `index`. A plain `resize(index + 1, tool_call.clone())` would
			// fill any GAP slots below `index` with a clone of THIS call's data instead of an
			// empty placeholder -- silently duplicating one call's name/args onto unrelated
			// indices if a later index streams before an earlier one. Grow with genuine empty
			// placeholders and set only the target slot; a legitimate delta for a gap index
			// still merges correctly via the `get_mut` branch above.
				...
				Some(Ok(Event::Message(message))) => {
					...
					if let Some(mut first_choice) = first_choice {
							...
							&& delta_tool_calls.as_array().is_some_and(|calls| !calls.is_empty())
						{
							...
							// original bug) silently dropped every other call in the batch; a LATER
							// arguments-only delta for that dropped index then found no existing slot
							// and fell into `capture_tool_call`'s "new tool call" branch with an empty
