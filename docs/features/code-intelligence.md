...
| Watcher feeding itself | inotify reports `IN_OPEN`, so the reindexer's own reads are events for the same file. The handler drops every access-only event (open / read / close-without-write; `Close(Write)` is kept) before it reaches the worker — without this the watcher pinned a core per process for its whole lifetime once the initial walk seeded it. Regression test: `the_reindexers_own_reads_do_not_feed_the_watcher`. |
| Watch-set size | One non-recursive watch per directory the indexer walks (`source_walker`, shared with `update()`), never a recursive watch on the root — that registered ~55,000 watches here (`target/`, `node_modules/`, worktrees) and woke the watcher for every sibling cargo build. New directories are registered by the worker as they appear (`Change::NewDir`). |
