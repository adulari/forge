...
| **Session replay** — **done** | Record prompts + model versions + outputs; replay + diff; auditable, reproducible AI. **Shipped:** `forge replay <id>` (turn-by-turn transcript) + `forge replay <a> <b>` (summary diff + per-turn content diff) + `/replay` in-session chat command + `forge replay <id> --json` (JSON export) + `forge replay <id> --rerun` (re-execute the recorded user prompts on the current model/mesh in a fresh session, then auto-diff against the original — tools gated by the normal permission mode) + `forge replay <id> --html <path>` / `/export [path]` (one self-contained HTML file of the transcript). | [session-replay.md](features/session-replay.md); Helm note |
| **Side questions** — **done** | `/btw <question>` (alias `/side`) — a one-off model call whose answer never joins the transcript or history, rendered as a distinct side-note card. Deliberately stateless (no side-conversation memory across calls) — see doc for why. | [side-questions.md](features/side-questions.md) |
