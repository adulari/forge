...
- **Status:** Accepted
- **Date:** 2026-06-14
- **Clarified:** 2026-07-17 — “single binary” means one native executable with no language runtime,
  not a fully static ELF/PE/Mach-O with no platform-library dependencies.
...
2. **Cargo workspace of library crates + one thin binary** — each subsystem (provider,
   ...
   explicit public API; the binary wires them together. Compiles to one native
...
assembled by one thin `forge` binary crate that produces a single native executable.
