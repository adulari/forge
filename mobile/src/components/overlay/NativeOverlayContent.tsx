...
// (handoff pattern 5): `MeshExplainFromOverlay` (components/session/MeshExplain.tsx) renders
// the picked-model card + ranked reject rows, and this file layers the budget meter + fallback
// chain parsed tolerantly from `overlay.body` underneath it. `overlay:workflow` rows speak the
// agent-row language (pattern 2): status medallion + accent/danger text for running/failed.
// `overlay:usage` is unchanged.
...
import { workflowState } from "./meshParse";
...
import { MeshExplainFromOverlay } from "../session/MeshExplain";
...
// ---------------------------------------------------------------------------
// overlay:mesh — MeshExplainFromOverlay (picked-model card + ranked rejects) +
// budget/fallback chain parsed tolerantly from `overlay.body`
// ---------------------------------------------------------------------------
...
function MeshRows({ overlay, onSelect }: NativeOverlayRowsProps) {
  ...
  return (
    <ScrollView style={styles.rows} keyboardShouldPersistTaps="handled">
      <MeshExplainFromOverlay overlay={overlay} onSelectRow={onSelect} />
