...
function ConfigFieldRow({ field, scope }: { field: ConfigField; scope: Scope }) {
  ...
  if (field.field_type === "list") {
    ...
    return <View style={styles.field}><Text style={[type.body, { color: tokens.ink }]}>{field.label}</Text><Text style={[type.sub, { color: error ? tokens.danger : tokens.ink3 }]}>{subtitle}</Text>{values.map((value, index) => <View key={`${field.key}-${index}`} style={styles.listItem}><Input containerStyle={styles.listInput} value={value} onChangeText={(next) => { const updated = [...values]; updated[index] = next; setDraft(JSON.stringify(updated)); }} onBlur={(event) => { const updated = [...values]; updated[index] = event.nativeEvent.text; save(JSON.stringify(updated)); }} accessibilityLabel={`${field.label} item ${index + 1}`} autoCapitalize="none" autoCorrect={false} /><Pressable onPress={() => { const next = JSON.stringify(values.filter((_, itemIndex) => itemIndex !== index)); setDraft(next); save(next); }} accessibilityRole="button" accessibilityLabel={`Remove ${field.label} item ${index + 1}`}><Text style={[styles.reset, { color: tokens.danger }]}>Remove</Text></Pressable></View>)}<Pressable onPress={() => { const next = JSON.stringify([...values, ""]); setDraft(next); }} accessibilityRole="button" accessibilityLabel={`Add ${field.label} item`}><Text style={[styles.reset, { color: tokens.accent }]}>Add item</Text></Pressable></View>;
  }
  if (field.field_type === "json") {
    return <View style={styles.field}><Input label={field.label} value={draft} onChangeText={setDraft} onBlur={(event) => save(event.nativeEvent.text)} multiline numberOfLines={8} autoCapitalize="none" autoCorrect={false} mono error={error ?? undefined} accessibilityLabel={field.label} /><Text style={[type.sub, { color: tokens.ink3 }]}>{field.help ?? `${field.key} · ${field.source}`}</Text></View>;
  }
  ...
  return <View style={styles.field}><Input label={field.label} value={draft} onChangeText={setDraft} onBlur={(event) => { if (event.nativeEvent.text !== field.value) save(event.nativeEvent.text); }} keyboardType={field.field_type === "int" || field.field_type === "float" ? "decimal-pad" : "default"} autoCapitalize="none" autoCorrect={false} error={error ?? undefined} accessibilityLabel={field.label} /><Text style={[type.sub, { color: tokens.ink3 }]}>{field.help ?? `${field.key} · ${field.source}`}</Text>{field.modified ? <Pressable onPress={() => save(undefined)} accessibilityRole="button" accessibilityLabel={`Reset ${field.label}`}><Text style={[styles.reset, { color: tokens.accent }]}>Reset to default ({field.default || "empty"})</Text></Pressable> : null}</View>;
}
