...
export default function ModelsScreen() {
  ...
  const models = useMemo(() => (query.data?.providers ?? []).flatMap(({ models: rows }) => rows).filter((model) => !needle || `${model.name} ${model.id} ${tierFor(model)}`.toLocaleLowerCase().includes(needle)).sort((a, b) => TIER_ORDER.indexOf(tierFor(a)) - TIER_ORDER.indexOf(tierFor(b)) || (b.benchmark_intelligence ?? -Infinity) - (a.benchmark_intelligence ?? -Infinity) || a.name.localeCompare(b.name)), [needle, query.data?.providers]);
