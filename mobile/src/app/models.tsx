...
type ProviderModel = { provider: string; model: ModelRow };
type ModelListItem = { kind: "tier"; tier: Tier; count: number } | { kind: "model"; row: ProviderModel };
...
const contextLabel = (value: number | null | undefined) => value == null ? null : new Intl.NumberFormat("en", { notation: "compact", maximumFractionDigits: 1 }).format(value);

const ModelRowItem = memo(function ModelRowItem({ row }: { row: ProviderModel }) {
  const tokens = useTokens();
  const [open, setOpen] = useState(false);
  const { provider, model } = row;
  const ready = !model.health;
  // Prototype meta line (mobile.dc.html Models): LEFT `provider · access [· frontier]`,
  // RIGHT `IQ 74.2 · code 76.8 · 200k` — never the full model id (that's what made rows
  // truncate into unreadable "…" soup).
  const access = model.subscription ? "subscription" : model.free ? "free" : "api";
  const left = [provider, access, model.frontier ? "frontier" : null].filter(Boolean).join(" · ");
  const benchmark = model.benchmark_intelligence == null ? null : `IQ ${model.benchmark_intelligence.toFixed(1)} · code ${model.benchmark_coding?.toFixed(1) ?? "—"}`;
  const right = [benchmark, contextLabel(model.context_window)].filter(Boolean).join(" · ");
  return <Pressable onPress={() => setOpen((value) => !value)} accessibilityRole="button" accessibilityState={{ expanded: open }} accessibilityLabel={`${model.name}, ${provider}, ${tierFor(model)} tier, ${ready ? "ready" : "benched"}`}><View style={[styles.model, { borderBottomColor: tokens.hairline }]}><View style={styles.row}><Text style={[styles.name, { color: ready ? tokens.ink : tokens.ink2 }]} numberOfLines={1}>{model.name}</Text><View style={[styles.dot, { backgroundColor: ready ? tokens.success : tokens.danger }]} /><Text style={[type.meta, { color: ready ? tokens.success : tokens.danger }]}>{ready ? "ready" : "benched"}</Text></View><View style={styles.metaRow}><Text style={[type.monoMeta, tabularNums, styles.tags, { color: tokens.ink4 }]} numberOfLines={1}>{left}</Text>{right ? <Text style={[type.monoMeta, tabularNums, { color: tokens.ink4 }]} numberOfLines={1}>{right}</Text> : null}</View>{model.health ? <Text style={[type.sub, styles.healthReason, { color: tokens.danger }]} numberOfLines={open ? 6 : 1}>{model.health.reason} · {retryLabel(model.health.until_epoch)}</Text> : null}</View></Pressable>;
});
...
function ModelsScreenBody() {
  ...
  const empty = query.isLoading ? <View style={styles.loading}><ActivityIndicator color={tokens.accent} /><Text style={[type.sub, { color: tokens.ink3 }]}>Loading model catalog…</Text></View> : query.isError ? <EmptyState icon={Cpu} message={`Could not load model health: ${query.error?.message || "unknown error"}. Pull to retry.`} /> : allServerModels.length > 0 ? <EmptyState icon={Cpu} message={search ? "No models match that search." : "Models are available but could not be organized."} /> : query.data?.catalog === "unavailable" ? <EmptyState icon={Cpu} message="No recent model catalog. Run forge models on this host to discover providers." /> : <EmptyState icon={Cpu} message="No models are available from this server." />;
  return <Screen scroll={false} contentContainerStyle={styles.screen}><BoundedList data={items} renderItem={renderItem} keyExtractor={(item) => item.kind === "tier" ? `tier:${item.tier}` : item.row.model.id} ListHeaderComponent={header} ListEmptyComponent={empty} refreshing={refresh.isPending} onRefresh={() => refresh.mutate()} contentContainerStyle={styles.content} /></Screen>;
}
