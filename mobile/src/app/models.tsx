...
const formatContext = (tokens: number | null) => tokens == null ? "context unknown" : `${new Intl.NumberFormat(undefined, { notation: "compact", maximumFractionDigits: 1 }).format(tokens)} context`;
const benchmark = (model: ModelRow) => model.benchmark_intelligence == null ? "unranked" : `IQ ${model.benchmark_intelligence.toFixed(1)} · code ${model.benchmark_coding?.toFixed(1) ?? "—"}`;

const ModelCard = memo(function ModelCard({ model }: { model: ModelRow }) { const tokens = useTokens(); const [open, setOpen] = useState(false); return <Pressable onPress={() => setOpen((value) => !value)} accessibilityRole="button" accessibilityState={{ expanded: open }} accessibilityLabel={`${model.name}, ${model.tier} tier, ${model.health ? "benched" : "ready"}`}><Card style={styles.model}><View style={styles.row}><View style={styles.name}><Text style={[type.body, { color: tokens.ink }]} numberOfLines={1}>{model.name}</Text><Text style={[type.sub, { color: tokens.ink3 }]} numberOfLines={1}>{model.id}</Text></View>{model.health ? <Badge label="benched" tone="danger" /> : <Badge label="ready" tone="success" />}</View><View style={styles.tags}><Badge label={model.tier} tone={model.tier === "complex" ? "warn" : model.tier === "standard" ? "accent" : "neutral"} />{model.free ? <Badge label="free" tone="success" /> : null}{model.paid ? <Badge label={model.estimated_cost_usd > 0 ? `~$${model.estimated_cost_usd.toFixed(2)}` : "metered"} tone="neutral" /> : null}</View><Text style={[type.meta, { color: tokens.ink3 }]} numberOfLines={1}>{`${benchmark(model)} · ${formatContext(model.context_window)}`}</Text>{model.health && open ? <Text style={[type.sub, { color: tokens.danger }]} numberOfLines={3}>{model.health.reason} · {retryLabel(model.health.until_epoch)}</Text> : null}</Card></Pressable>; });

export default function ModelsScreen() {
  const tokens = useTokens(); const query = useModels(); const [search, setSearch] = useState("");
  const normalized = search.trim().toLocaleLowerCase();
  const models = useMemo(() => (query.data?.providers ?? []).flatMap(({ models }) => models).filter((model) => !normalized || `${model.name} ${model.id} ${model.tier}`.toLocaleLowerCase().includes(normalized)).sort((a, b) => TIER_ORDER.indexOf(a.tier) - TIER_ORDER.indexOf(b.tier) || (b.benchmark_intelligence ?? -Infinity) - (a.benchmark_intelligence ?? -Infinity) || a.name.localeCompare(b.name)), [query.data?.providers, normalized]);
