...
export default function ModelsScreen() {
  ...
  return <Screen scroll refreshControl={<RefreshControl refreshing={query.isFetching} onRefresh={() => void query.refetch()} />} contentContainerStyle={styles.content}><Pressable onPress={() => router.back()} accessibilityRole="button"><Text style={[styles.back, { color: tokens.accent }]}>‹ Settings</Text></Pressable><Text style={[type.title, { color: tokens.ink }]}>Models & mesh health</Text><Text style={[type.sub, { color: tokens.ink3 }]}>Your discovered catalog and failover protection.</Text>{query.isError ? <Card><Text style={[type.body, { color: tokens.danger }]}>Could not load model health. Pull to retry.</Text></Card> : null}{query.data?.catalog === "unavailable" ? <Card><Text style={[type.body, { color: tokens.ink2 }]}>No recent model catalog yet. Run Forge on this host or use <Text style={{ color: tokens.accent }}>forge models</Text> to discover providers.</Text></Card> : null}{providers.map((provider) => <View key={provider.provider}><SectionHeader>{`${provider.provider} · ${provider.models.length} models`}</SectionHeader>{provider.models.map((model) => <ModelCard key={model.id} model={model} />)}</View>)}</Screen>;
}

const styles = StyleSheet.create({ content: { paddingTop: space.space12, paddingBottom: space.space32, gap: space.space12 }, back: { fontSize: 15, fontWeight: "600" }, model: { gap: space.space8, marginBottom: space.space8 }, row: { flexDirection: "row", alignItems: "center", gap: space.space8 }, name: { flex: 1, gap: 2 }, tags: { flexDirection: "row", flexWrap: "wrap", gap: space.space4 } });
