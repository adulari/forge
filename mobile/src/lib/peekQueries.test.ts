...
import {
  pastSessionsRefetchPolicy,
  sessionsRefetchPolicy,
  SESSIONS_RECOVERY_POLL_MS,
} from "./peekQueries";
...
describe("pastSessionsRefetchPolicy", () => {
  it("keeps a peeked History off the network", () => {
    expect(pastSessionsRefetchPolicy({ peeking: true })).toEqual({
      refetchOnMount: false,
      refetchInterval: false,
      refetchOnWindowFocus: false,
    });
  ...
  it("refreshes History when it is actually opened", () => {
    const policy = pastSessionsRefetchPolicy({ peeking: false });
    expect(policy.refetchOnMount).toBe(true);
    expect(policy.refetchOnWindowFocus).toBe(true);
  });

  it("never polls, peeked or not — History is not a live view", () => {
    expect(pastSessionsRefetchPolicy({ peeking: true }).refetchInterval).toBe(false);
    expect(pastSessionsRefetchPolicy({ peeking: false }).refetchInterval).toBe(false);
  });
});
